...
create_unique_string <-
  function(what, months, date = NULL, days_back, years_back, reference_years,
    ...
    if(!is.null(buffers)) {
      buffers_new <- sub('\\..*$', '', basename(as.character(buffers)))
      strings$buffers_string <- glue::glue("buffer={buffers_new}")
    }
...
download_and_wrangle <-
  function(what, year, file_name, folder_name, where_raw, days, months) {
    ...
    base_filename <- glue::glue("{file_name}_{year}_v6-1_de.nc")
        ...
        if (!file.exists(.x)) {
          file_to_download <-
            glue::glue(
              ...
              "daily/hyras_de/{folder_name[[.y]]}/{base_filename[[.y]]}"
            )
    ...
    if (what == "drought") {
      inverse <-
        function(x){ ((x - max(x, na.rm = TRUE)) * -1) + min(x, na.rm = TRUE) }
      ...
      dmi_temp <- raster_file[[1]] + 10
      dmi_temp[dmi_temp == 0] <- .00000001

      dmi_prec <- terra::aggregate(raster_file[[2]], fact = 5)

      raster_file <- dmi_prec / dmi_temp

      # invert
      raster_file <- spatialEco::raster.invert(raster_file)
...
download_and_wrangle_reference_layers <-
  function (
    what, years, years_back, reference_years, file_name, folder_name, where_raw,
...
create_dwd_weather_layer <-
  function(
    ...
    if (what == "drought") {
      folder_name <- c("air_temperature_mean", "precipitation")
      file_name <- c("tas_hyras_5", "pr_hyras_1")
    }
    ...
    if (!file.exists(unique_filename_plus_location) || isFALSE(cache)) {
      ...
      tmp_raster <-
        download_and_wrangle_reference_layers(
          what = what, years = years, years_back = years_back,
...
link_dwd_weather_single_date <- function (
  ...
  if(!file.exists(unique_variable_name_location)) {# || !is.null(where_secret)) {
    ...
    if (geometry_type == "POINT") {
      if (!grepl("[.][a-z]+", buffers)) {
        buffers <- as.numeric(buffers)
        ...
        tmp_geometries <-
          ...
          dplyr::mutate(duplicated_geometry = x)

        data <-
          sf::st_join(data, tmp_geometries, left = FALSE) |>
          sf::st_set_geometry("duplicated_geometry") |>
